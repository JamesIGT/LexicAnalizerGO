...
usuario_git = "dalay20"  # Cambia esto por tu usuario de GitHub
...
# === REGLAS SINTÁCTICAS ===
# Precedencia para operadores (opcional, pero útil)
precedence = (
    ('left', 'PLUS', 'MINUS'),
...
def p_statement(p):
    '''statement : declaration
                 ...
                 | type_declaration'''   # <-- Agrega esta línea
    logging.info("Regla: statement")

def p_declaration(p):
    '''declaration : VAR VARIABLE ASSIGN expression
                  | VAR VARIABLE ASSIGN expression SEMICOLON'''
    logging.info("Regla: declaration → VAR VARIABLE ASSIGN expression (; opcional)")

def p_assignment(p):
    '''assignment : variable ASSIGN expression
                 | variable ASSIGN expression SEMICOLON'''
    logging.info("Regla: assignment → variable ASSIGN expression (; opcional)")
...
def p_if_statement(p):
    ...
    logging.info("Regla: if_statement → IF expression block else_clause")

def p_else_clause(p):
    '''else_clause : ELSE block
                   | empty'''
    logging.info("Regla: else_clause → ELSE block | ε")
...
def p_for_loop(p):
    ...
    logging.info("Regla: for_loop tradicional y con declaración corta")
...
def p_function_call(p):
    ...
    logging.info("Regla: function_call → variable/APPEND/PRINTLN/PRINTF LPAREN argument_list? RPAREN")

def p_argument_list(p):
    '''argument_list : expression
                     | argument_list COMMA expression'''
    logging.info("Regla: argument_list")
...
def p_expression(p):
    '''expression : expression PLUS expression
                  ...
                  | TRUE
                  | FALSE'''
    logging.info("Regla: expresión aritmética/literal/relacional/lógica")
...
def p_error(p):
    if p:
        ...
        # Agrega esto para mostrar contexto:
...
def p_case(p):
    ...
    logging.info("Regla: case o default")
...
def p_variable(p):
    ...
    logging.info("Regla: acceso a variable y por índice")
...
def p_short_declaration(p):
    ...
    logging.info("Regla: short declaration múltiple → variable_list := expression_list ;")

def p_variable_list(p):
    '''variable_list : variable
                     | variable COMMA variable_list'''
    logging.info("Regla: variable_list")

def p_expression_list(p):
    '''expression_list : expression
                       | expression COMMA expression_list'''
    logging.info("Regla: expression_list")

def p_expression_type_cast(p):
    '''expression : LBRACKET RBRACKET variable LPAREN expression RPAREN'''
    logging.info("Regla: conversión de tipo → []tipo(expr)")
...
def p_expression_type_cast_simple(p):
    ...
    logging.info("Regla: conversión de tipo simple → tipo(expr)")

def p_type_declaration(p):
    '''type_declaration : TYPE VARIABLE STRUCT LBRACE struct_fields RBRACE'''
    logging.info("Regla: declaración de tipo struct")

def p_struct_fields(p):
    '''struct_fields : struct_fields struct_field
                    | struct_field
                    | empty'''
    logging.info("Regla: lista de campos de struct")

def p_struct_field(p):
    '''struct_field : VARIABLE type
                   | empty'''
    logging.info("Regla: campo de struct o vacío")

def p_expression_struct_literal(p):
    '''expression : VARIABLE LBRACE field_list RBRACE'''
    logging.info("Regla: inicialización de struct → TIPO{...}")

def p_field_list(p):
    '''field_list : field_list COMMA field
                  | field
                  | empty'''
    logging.info("Regla: lista de campos de struct literal")

def p_field(p):
    '''field : VARIABLE COLON expression'''
    logging.info("Regla: campo de struct literal")
